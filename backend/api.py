...
def actualizar_producto(id_producto):
    ...
    return jsonify({"id": id_producto, "nombre": nombre, "precio": precio}), 200
...
def actualizar_pedido(id_pedido):
    ...
    cursor = conexion.cursor()
    cursor.execute(
        "UPDATE pedidos SET id_cliente = %s, fecha = %s WHERE id = %s",
        (id_cliente, fecha, id_pedido)
    )
    conexion.commit()
    cursor.close()
    conexion.close()

    return jsonify({"id": id_pedido, "id_cliente": id_cliente, "fecha": fecha}), 200
...
@app.route("/pedidos/<int:id_pedido>/detalle")
def obtener_detalle_pedido(id_pedido):
    ...
    detalle = []
    for fila in resultados:
        detalle.append({"producto": fila[0], "cantidad": fila[1], "precio": float(fila[2])})
